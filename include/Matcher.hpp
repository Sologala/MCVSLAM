...
class Matcher {
   ...
   public:
    ...
    static MatchResKnn DBowMatch(const Desps &desp1, const DBoW2::FeatureVector &bow_feat1, const Desps &desp2,
                                 const DBoW2::FeatureVector &bow_feat2);

    // knn match with check kp error
    static MatchResKnn KnnMatch(const std::vector<cv::KeyPoint> &kps1, const std::vector<cv::Mat> &desp1, const std::vector<cv::KeyPoint> &kps2,
                                const std::vector<cv::Mat> &desp2, const std::vector<float> sigma2, int k = 2, uint n_threads = 0);
