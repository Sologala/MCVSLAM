...
MatchRes &MatchRes::FilterOrientation(const Keypoints &kps1, const Keypoints &kps2) {
    ...
    static auto cmp = [](const vector<uint> &a, const vector<uint> &b) { return a.size() > b.size(); };
...
MatchResKnn Matcher::KnnMatch(const std::vector<cv::KeyPoint> &kps1, const std::vector<cv::Mat> &desp1, const std::vector<cv::KeyPoint> &kps2,
                              const std::vector<cv::Mat> &desp2, const std::vector<float> sigma2, int k, uint n_threads) {
    class _LoopBody : public cv::ParallelLoopBody {
       public:
        ...
        bool is_swap;
        uint size = 0;
        _LoopBody(std::vector<cv::Mat> &_desp1, std::vector<cv::Mat> &_desp2, std::vector<std::vector<cv::DMatch>> &res_)
...
class LoopBody : public cv::ParallelLoopBody {
   public:
    ...
    LoopBody(const std::vector<cv::Mat> &_desp1, const std::vector<cv::Mat> &_desp2, std::vector<std::vector<cv::DMatch>> &res_, bool is_swap)
        : desp1(_desp1), desp2(_desp2), res(res_) {}
    virtual void operator()(const cv::Range &r) const {
        for (int i = r.start; i != r.end; i++)  //遍历
        {
