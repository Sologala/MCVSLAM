...
MatchRes &MatchRes::Filter_GMS(const cv::Size sz1, const cv::Size sz2,
                               const Keypoints &kps1, const Keypoints &kps2,
                               uint threshold) {
  MatchRes ret;
  cv::xfeatures2d::matchGMS(sz1, sz2, kps1, kps2, *this, ret, false, false,
                            threshold);
  *this = ret;
  return *this;
}
...
MatchRes &MatchRes::FilterOrientation(const Keypoints &kps1,
                                      const Keypoints &kps2) {
  std::vector<uint> rot_bins[HISTO_LENGTH];
  for (int i = 0; i < HISTO_LENGTH; i++)
    rot_bins[i].reserve(500);
  for (uint i = 0, sz = this->size(); i < sz; i++) {
    ...
    if (bin_id == HISTO_LENGTH)
      bin_id = 0;
    assert(bin_id >= 0 && bin_id < HISTO_LENGTH);
    rot_bins[bin_id].push_back(i);
  }
  static auto cmp = [](const vector<uint> &a, const vector<uint> &b) {
    return a.size() > b.size();
  };
  sort(&rot_bins[0], &rot_bins[0] + HISTO_LENGTH, cmp);
  // pick minnal 3 size()
  MatchRes ret;
  for (uint i = 0; i < 3; i++) {
...
MatchRes &MatchRes::FilterFMatrix(const Keypoints &kps1, const Keypoints &kps2,
                                  const cv::Mat &F12,
...
MatchRes &MatchRes::Show(const std::string &wnd_name, ObjectRef &obj1,
                         ObjectRef &obj2) {
  cv::Mat _show_img;
...
MatchRes MatchResKnn::FilterRatio(const float ratio) {
  ...
  for (const auto &v_match : (*this)) {
    ...
    } else if (v_match.size() >= 2 &&
               v_match[0].distance / v_match[1].distance <= ratio) {
      res.push_back(v_match[0]);
    }
  }
...
cv::Ptr<cv::BFMatcher> matcher_L2 =
    cv::BFMatcher::create(cv::NormTypes::NORM_L2);
cv::Ptr<cv::BFMatcher> matcher_HAM =
...
MatchResKnn Matcher::KnnMatch_cv(cv::InputArray desp1, cv::InputArray desp2,
                                 int k) {
  MatchResKnn res;
...
MatchResKnn Matcher::DBowMatch(const Desps &desp1,
  ...
  while (f1_it != f1_it_end && f2_it != f2_it_end) {
    if (f1_it->first == f2_it->first) {
      ...
      for (uint f1_kp_idx : f1_kp_idxs) {
        ...
        if (d[1] != 999) {
          res.push_back({cv::DMatch(f1_kp_idx, d_idx[0], d[0]),
                         cv::DMatch(f1_kp_idx, d_idx[1], d[1])});
        }
      }
...
MatchResKnn Matcher::KnnMatch(const std::vector<cv::KeyPoint> &kps1,
                              const std::vector<cv::Mat> &desp1,
                              const std::vector<cv::KeyPoint> &kps2,
                              const std::vector<cv::Mat> &desp2,
                              const std::vector<float> sigma2, int k,
                              uint n_threads) {
  class _LoopBody : public cv::ParallelLoopBody {
  public:
    std::vector<cv::Mat> &desp2;
    std::vector<cv::Mat> &desp1;
    std::vector<std::vector<cv::DMatch>> &res;
    bool is_swap;
    uint size = 0;
    _LoopBody(std::vector<cv::Mat> &_desp1, std::vector<cv::Mat> &_desp2,
              std::vector<std::vector<cv::DMatch>> &res_)
        : desp1(_desp1), desp2(_desp2), res(res_) {
      // check largest
      if (_desp1.size() > _desp2.size()) {
        desp1 = _desp1, desp2 = _desp2;
        is_swap = false;
      } else {
        desp2 = _desp1, desp1 = _desp2;
        is_swap = true;
      }
      size = desp1.size();
    }
...
public:
  ...
  LoopBody(const std::vector<cv::Mat> &_desp1,
           const std::vector<cv::Mat> &_desp2,
           std::vector<std::vector<cv::DMatch>> &res_, bool is_swap)
      : desp1(_desp1), desp2(_desp2), res(res_) {}
  virtual void operator()(const cv::Range &r) const {
    for (int i = r.start; i != r.end; i++) //遍历
    {
      ...
      if (!is_swap) {
        ...
        res[i][0] = cv::DMatch(i, d[0], d_idx[0]);
        res[i][1] = cv::DMatch(i, d[1], d_idx[1]);
      }
    }
  }
};

MatchResKnn Matcher::KnnMatch(const std::vector<cv::Mat> &desp1,
                              const std::vector<cv::Mat> &desp2, int k,
                              uint n_threads) {
  assert(desp1.size());
  k = min(static_cast<int>(desp2.size()), k);
  MatchResKnn ret;
  uint size = max(desp1.size(), desp2.size());
  bool is_swap = (desp1.size() < desp2.size() ? true : false);
  is_swap = false;
  size = desp1.size();
  ret.resize(size, std::vector<cv::DMatch>(2));
  LoopBody *body;

  if (!is_swap)
    body = new LoopBody(desp1, desp2, ret, false);
  else {
    body = new LoopBody(desp2, desp1, ret, true);
  }
  cv::parallel_for_(cv::Range(0, size), *body); //启动
  delete body;
  return ret;
}

MatchResKnn Matcher::KnnMatch(const cv::Mat &desp1, const cv::Mat &desp2, int k,
                              uint n_threads) {
  MatchResKnn res;
  Get_BF_Matcher(MATCH_DISTANCE::HAMMING)->knnMatch(desp1, desp2, res, k);
  return res;
