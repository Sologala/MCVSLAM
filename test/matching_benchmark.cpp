...
Pinhole cam_left("/home/wen/SLAM/MCVSLAM/config/camleft.yaml"),
    cam_wide("/home/wen/SLAM/MCVSLAM/config/camwide.yaml");
...
void track(std::vector<cv::Mat> &imgs, double time_stamp) {
  ...
  KeyFrame cur_frame =
      _map.CreateFrame(imggrays[0], imggrays[1], imggrays[2], time_stamp,
                       &cam_left, &cam_left, &cam_wide, {});
...
int main(int argc, char **argv) {
  ...
  argPaser.add<string>("configPath", 'c', "config Path", false,
                       "../config/capture.yaml");
